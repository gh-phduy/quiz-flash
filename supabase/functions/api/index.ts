...
app.post('/api/review/record', async (c) => {
  ...
  const { cardId, quality, mode } = await c.req.json()
  const result = await recordCardReview(supabase, user.id, cardId, quality, mode)
...
app.post('/api/review/record-bulk', async (c) => {
  ...
  const result = await recordBulkCardReviews(supabase, user.id, reviews)
  return c.json(result)
})
...
app.get('/api/review/due', async (c) => {
  ...
  const result = await getDueCardsToReview(supabase, user.id)
  return c.json(result)
})
