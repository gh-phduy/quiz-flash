...
BEGIN
    ...
    ELSE
        -- Trên 25%: Ưu tiên 30% từ mới, 70% ôn tập
    ...
    -- Nếu thẻ mới không đủ, dồn ngược lại cho thẻ cũ
    ...
    old_cards AS (
        ...
        ORDER BY 
            -- 1. Tỷ lệ đúng thấp nhất lên đầu (Accuracy ASC)
            (r.correct_count::float / NULLIF(r.total_reviews, 0)) ASC NULLS FIRST,
            -- 2. Nếu tỷ lệ bằng nhau, ưu tiên từ làm nhiều lần (tức sai ngoan cố)
