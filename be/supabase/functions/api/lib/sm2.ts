...
export function calculateSM2(
  ...
  if (mode === 'typing') {
    ...
    const nextReviewDateStr = prevNextReviewDateStr || clientLocalDateStr || formatDateToYYYYMMDD(new Date());
    const nextReviewDate = new Date(nextReviewDateStr + 'T00:00:00Z');
  ...
  let nextReviewDate = clientLocalDateStr ? new Date(clientLocalDateStr + 'T00:00:00Z') : new Date();
  let nextReviewDateStr = '';

  const shouldFreeze = !inReviewSession && isCorrect && prevRepetitions >= 1;

  if (shouldFreeze && prevNextReviewDateStr) {
    nextReviewDate = new Date(prevNextReviewDateStr + 'T00:00:00Z');
    nextReviewDateStr = prevNextReviewDateStr;
  } else {
    nextReviewDate.setUTCDate(nextReviewDate.getUTCDate() + daysToAdd);
