...
import { GameModeType } from '../lib/sm2.ts';

export async function generateGameSession(supabase: SupabaseClient, userId: string, setId: string, totalCardsToLearn: number = 20, mode?: string) {
  try {
        
    
    // Try calling new Smart Waterfall RPC first
    const { data: cards, error: rpcError } = await supabase.rpc('get_smart_waterfall_cards_v5', {
      p_set_id: setId,
      p_user_id: userId,
      p_total_limit: totalCardsToLearn,
      p_game_mode: mode || 'flashcards'
    });

    if (rpcError) {
      console.error('RPC Error calling get_smart_waterfall_cards_v5:', rpcError);
    }
...
export async function checkNewCardsForSession(supabase: SupabaseClient, userId: string, cardIds: string[]) {
...
export async function updateGameScores(supabase: SupabaseClient, userId: string, correctCardIds: string[], incorrectCardIds: string[] = []) {
  try {
...
export async function logGameSession(supabase: SupabaseClient, userId: string, params: {
  setId: string;
  mode: GameModeType;
  totalCards: number;
  correctCount: number;
