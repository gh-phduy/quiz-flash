...
import { calculateSM2, formatDateToYYYYMMDD, GameModeType } from '../lib/sm2.ts';
...
export async function recordBulkCardReviews(
  ...
  try {
    ...
    supabase
      ...
      .then(({ count }) => {
        if (count && count > 0) {
          supabase.from('profiles').update({ words_learned: count }).eq('id', userId).then(() => {});
        }
      });
...
export async function getStatusDashboard(supabase: SupabaseClient, targetUserId: string) {
  try {
    ...
    const modePerformance: Record<string, { correct: number; total: number; accuracy: number }> = {
      ...
      typing: { correct: 0, total: 0, accuracy: 0 },
      review: { correct: 0, total: 0, accuracy: 0 },
    };
...
export async function getUpcomingReviews(supabase: SupabaseClient, targetUserId: string) {
  try {
    const targetId = targetUserId;
    if (!targetId) return [];
...
export async function getDueCardsToReview(supabase: SupabaseClient, userId: string, clientLocalDateStr?: string) {
  try {
    if (!userId) return [];

    const todayStr = clientLocalDateStr || formatDateToYYYYMMDD(new Date());
...
export async function testReviewSingleCard(supabase: SupabaseClient, userId: string, targetCardId?: string) {
  try {
    ...
    const res = await recordCardReview(supabase, userId, cardIdToReview, 4, 'test');
...
export async function testUpdateRepetitionsToTwo(supabase: SupabaseClient, userId: string) {
  try {
    ...
    if (!updatedRows || updatedRows.length === 0) {
      ...
      return { 
        success: false, 
        error: `Không có dòng nào được cập nhật. Debug: user_id=${userId}, review_id=${currentReview.id}, updatedRows=${JSON.stringify(updatedRows)}, updateErr=${JSON.stringify(updateErr)}` 
      };
    }
